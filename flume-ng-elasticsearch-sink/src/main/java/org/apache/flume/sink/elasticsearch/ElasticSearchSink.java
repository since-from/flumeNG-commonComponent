...
import org.apache.flume.sink.elasticsearch.client.CommonUtils;
...
public class ElasticSearchSink extends AbstractSink implements Configurable {
  ...
  private int esIndexTTL = ElasticSearchSinkConstants.ES_TTL_DEFAULT;
  ...
  private IndexNameBuilder indexNameBuilder_exceptionLog;
  ...
  public IndexNameBuilder getIndexNameBuilder_exceptionLog() {
    return indexNameBuilder_exceptionLog;
  }

  public void setIndexNameBuilder_exceptionLog(IndexNameBuilder indexNameBuilder_exceptionLog) {
    this.indexNameBuilder_exceptionLog = indexNameBuilder_exceptionLog;
  }
  ...
  public Status process() throws EventDeliveryException {
    ...
    try {
      ...
      for (count = 0; count < batchSize; ++count) {
        ...
        //normal log and large tag
        String eventBody = new String(event.getBody(),"utf-8");

        list.add(eventBody);

        logger.debug("ES sink received event:"+eventBody);

        if(CommonUtils.isBadJson(eventBody)){
          continue;
        }

        JSONObject jsonObject = JSONObject.parseObject(eventBody);

        if(jsonObject.containsKey("ServiceName")){
          String serviceName = jsonObject.getString("ServiceName");
          if(StringUtils.isNotEmpty(serviceName) && serviceName.length() > 10000){
            serviceName = serviceName.substring(0,10000);
        ...
        //check normal log and exception log
        if(jsonObject.containsKey(ElasticSearchSinkConstants.ES_LOGTYPE_EXCEPTION_KEY)){
            client.addEvent(event, indexNameBuilder_exceptionLog, ElasticSearchSinkConstants.INDEX_EXCEPTION_NAME, ttlMs);
            index ++ ;
        }else {
             logger.debug("Normal log:"+eventBody);
             client.addEvent(event, indexNameBuilder, indexName, ttlMs);
             index ++ ;
        }

        //only handle long log.
//        if(jsonObject.containsKey(ElasticSearchSinkConstants.ES_LOGTYPE_LARGE_KEY)){
//          int expectDays = jsonObject.getInteger(ElasticSearchSinkConstants.ES_LOGTYPE_LARGE_KEY);
//          if(expectDays > esIndexTTL){
//            logger.debug("Long log:"+eventBody);
//            client.addEvent(event, indexNameBuilder, indexName, ttlMs);
//
//            index ++ ;
//          }
...
//        }
      ...
      if (count <= 0 || index == 0) {
        ...
        logger.debug("write event success.");
  ...
  public void configure(Context context) {
    ...
    if (StringUtils.isNotBlank(context.getString(ElasticSearchSinkConstants.ES_TTL_CONFIG))) {
       this.esIndexTTL = Integer.parseInt(context.getString(ElasticSearchSinkConstants.ES_TTL_CONFIG));
    }
    ...
    Context indexnameBuilderContext_exceptionLog = new Context();
    ...
    try {
      ...
      //for exception log
      indexNameBuilder_exceptionLog = clazz.newInstance();
      indexnameBuilderContext_exceptionLog.put(INDEX_NAME,ElasticSearchSinkConstants.INDEX_EXCEPTION_NAME);
      indexNameBuilder_exceptionLog.configure(indexnameBuilderContext_exceptionLog);
